import math
wine_percent = 0.40             # 40% от реколтата за производство на вино
x = int(input())        # кв.м е лозето – цяло число в интервала [10 … 5000]
y = float(input())      # грозде за един кв.м – реално число в интервала [0.00 … 10.00]
z = int(input())        # нужни литри вино – цяло число в интервала [10 … 600]
workers = int(input())  # работници – цяло число в интервала [1 … 20]
# 1 liters = 2.5 kg grapes

production = x * y
free_grapes_for_wine = production * wine_percent
wine_litters = free_grapes_for_wine / 2.5
difference = abs( wine_litters - z)

if wine_litters >= z:
    print(f"Good harvest this year! Total wine: {math.floor(wine_litters)} liters.")
