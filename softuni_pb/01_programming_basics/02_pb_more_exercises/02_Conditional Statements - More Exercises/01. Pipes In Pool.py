...
p1_debit = p1 * h
p2_debit = p2 * h
total_debit = p1_debit + p2_debit

p1_percent = p1_debit / total_debit
p2_percent = p2_debit / total_debit
pool_percent = total_debit / v
debit_done = total_debit - v

if debit_done <= 0:
    print(f"The pool is {pool_percent * 100:.2f}% full. "
          f"Pipe 1: {p1_percent * 100:.2f}%."
          f"Pipe 2: {p2_percent *100:.2f}%.")
else:
    print(f"For {h} hours the pool overflows with {debit_done:.2f} liters.")
