purpose = int(input()) # цел
max_purpose = purpose - 30
jumps_counter = 0
...
while unsuccessful_jumps < 3:
    ...
    if tray > max_purpose:
        unsuccessful_jumps = 0
        max_purpose += 5
        if max_purpose > purpose:
            print(f"Tihomir succeeded, he jumped over {purpose}cm after {jumps_counter} jumps.")
            break
    else:
        unsuccessful_jumps += 1

else:
    print(f"Tihomir failed at {max_purpose}cm after {jumps_counter} jumps.")
